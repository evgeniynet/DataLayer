...
{
    /// <summary>
    /// Summary description for Rpts.
    /// </summary>
    public class Rpts : DBAccess
    {
            ...
            DateTime startDate, DateTime endDate)
        {
            ...
            string _sqlWhere = "WHERE T.company_id=" + DId.ToString() + " AND ((T.CreateTime BETWEEN '" + Functions.FormatSQLDateTime(startDate) + "' AND '" + Functions.FormatSQLDateTime(endDate) + "') OR (T.ClosedTime BETWEEN '" + Functions.FormatSQLDateTime(startDate) + "' AND '" + Functions.FormatSQLDateTime(endDate) + "' AND T.Status='Closed')) ";
            ...
            string _sqlSelectFields = "SUM(CASE WHEN T.CreateTime BETWEEN '" + Functions.FormatSQLDateTime(startDate) + "' AND '" + Functions.FormatSQLDateTime(endDate) + "' THEN 1 ELSE 0 END) AS TotalCount, SUM(CASE T.status WHEN 'Open' THEN 1 ELSE 0 END) AS OpenCount, "
                      + "SUM(CASE WHEN T.status='Closed' AND T.ClosedTime BETWEEN '" + Functions.FormatSQLDateTime(startDate) + "' AND '" + Functions.FormatSQLDateTime(endDate) + "' THEN 1 ELSE 0 END) AS ClosedCount, "
                      + "SUM(CASE T.status WHEN 'On Hold' THEN 1 ELSE 0 END) AS HoldCount, "
                      + "SUM(CASE T.status WHEN 'Parts On Order' THEN 1 ELSE 0 END) AS PartsCount, ";
            if (IsSLA)
            {
                ...
                _sqlSelectFields += "AVG(cast(CASE T.status WHEN 'Open' THEN dbo.fxGetOperationalMinutes(" + DId.ToString() + ",T.CreateTime, getutcdate(), @WorkDays, @StartBusinnessTime, @EndBusinnessTime) ELSE NULL END as bigint)) AS AvgOpen, "
                        + "AVG(cast(CASE WHEN T.status='Closed' AND T.ClosedTime BETWEEN '" + Functions.FormatSQLDateTime(startDate) + "' AND '" + Functions.FormatSQLDateTime(endDate) + "' THEN dbo.fxGetOperationalMinutes(" + DId.ToString() + ",T.createtime,T.closedtime, @WorkDays, @StartBusinnessTime, @EndBusinnessTime) ELSE null END as bigint)) AS AvgClosed, "
            ...
            else
            {
                _sqlSelectFields += "AVG(cast(CASE T.status WHEN 'Open' THEN DATEDIFF(minute, T.createtime, getutcdate()) ELSE null END as bigint)) AS AvgOpen, "
                      + "AVG(cast(CASE WHEN T.status='Closed' AND T.ClosedTime BETWEEN '" + Functions.FormatSQLDateTime(startDate) + "' AND '" + Functions.FormatSQLDateTime(endDate) + "' THEN DATEDIFF(minute, T.createtime, T.closedtime) ELSE null END as bigint)) AS AvgClosed, "
            ...
            switch (filter.YAxis)
            {
                case Grouping.Account:
                    _sqlSelect = "SELECT ISNULL(A.Id,-1) AS ID, MAX(A.vchName) AS YAxis, " + _sqlSelectFields
                        + "FROM tbl_ticket T LEFT OUTER JOIN Accounts A ON A.DId=" + DId.ToString() + " AND A.Id=T.intAcctId ";
                    _sqlGroup = "GROUP BY A.id ";
                    break;
                case Grouping.AccountLocation:
                    _sqlSelect = "SELECT  ISNULL(T.intAcctId,-1) AS ID, ISNULL(T.AccountLocationId,-1) AS SubID, MAX(ISNULL(dbo.fxGetAccountName(" + DId.ToString() + ", T.intAcctId),'') + ISNULL(' / ' + dbo.fxGetUserLocationName(" + DId.ToString() + ", T.AccountLocationId),'')) AS YAxis, " + _sqlSelectFields
                        + "FROM tbl_ticket T ";
                    _sqlGroup = "GROUP BY T.intAcctId, T.AccountLocationId ";
                    break;
                case Grouping.Class:
                ...
                case Grouping.Technician:
                    _sqlSelect = "SELECT ISNULL(LJT.id,-1) AS ID, MAX(L.LastName+', '+L.FirstName) AS YAxis, " + _sqlSelectFields
                        + "FROM tbl_ticket T "
            ...
            else if (filter.AccountID < 0) _sqlWhere += "AND T.intAcctId IS NULL ";
            ...
            for (int i = 0; i < _DT.Rows.Count; i++)
            {
                ...
                isRecursive = true;
                _subDT = TicketsCount(DId, _f, OnHoldStatus, PartsTracking, SortExpession, GlobalFilterSQL, IsSLA, startDate, endDate);
                isRecursive = false;
